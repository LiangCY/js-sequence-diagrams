...
Raphael.fn.handRect = function (x, y, w, h) {
  assert(_every([x, y, w, h], _isFinite), 'x, y, w, h must be numeric');
  return this.path('M' + x + ',' + y + this.wobble(x, y, x + w, y) + this.wobble(x + w, y, x + w, y + h) + this.wobble(x + w, y + h, x, y + h) + this.wobble(x, y + h, x, y)).attr(RECT);
};
...
_extend(BaseTheme.prototype, {
  ...
  draw_title: function () {
    var title = this._title;
    if (title) this.draw_text_box(title, title.message, TITLE_MARGIN, TITLE_PADDING, this._font);
  },
  ...
  draw_actors: function (offsetY) {
    ...
    _each(this.diagram.actors, function (a) {
      ...
      // Veritical line
      var aX = getCenterX(a);
      var line = this.draw_line(aX, y + this._actors_height - ACTOR_MARGIN, aX, y + this._actors_height + ACTOR_MARGIN + this._signals_height);
      line.attr(LINE);
    }.bind(this));
  },
